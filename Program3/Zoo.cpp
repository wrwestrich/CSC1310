...
int main(){
    ...
    do{
        ...
        switch(input){
            case 1:
                cout << endl << endl << "Choose how you would like to enter the data:" << endl;
...
void EnterCreature(list<Creature>* List){
    ...
    do{
        cout << endl << "Enter the creature's name: ";
        getline(cin, tempName);

        cout << endl << "Enter a description:" << endl;
        getline(cin, tempDes);

        cout << endl << "Is the creature dangerous(y or n)? ";
        getline(cin, temp);

        cout << endl << "Enter cost per month: ";
        cin >> tempCost;
        cin.ignore();
        ...
        cout << endl << tempName << " has been added." << endl << endl;
...
void EnterCreatureFromFile(list<Creature>* List){
    ...
    cout << endl << "Enter the name of the file(example.txt): ";
    getline(cin, temp);

    inF.open(temp, std::fstream::in);
    if(!inF.is_open()){
        cout << endl << "File could not be found." << endl << endl;
        return;
    }
    ...
    cout << endl << numAdded << " creatures were added from the file." << endl;
...
void DeleteCreature(list<Creature>* List){
    ...
    cout << endl << "choose the creature you'd like to remove:" << endl;
    cin >> remove;

    List->DeleteNode(remove-1);

    cout << endl << "The creature has been removed" << endl << endl;
}
...
void PrintCreature(list<Creature>* List){
    ...
    if(size == 0){
        cout << endl << "There are no creatures in the zoo." << endl;
        return;
    }
    cout << endl;
    for(size_t i = 0; i < List->GetLength(); ++i){
...
void SaveToFile(list<Creature>* List){
    ...
    cout << endl << "Enter the name of the file you'd like to save to(example.txt):" << endl;
    getline(cin, name);
